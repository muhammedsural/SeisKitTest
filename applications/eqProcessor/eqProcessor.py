...
import pandas as pd
from applications.eqProcessor.record import ASC, AT2
from applications.eqProcessor.processFunctions import detrendFunction, filterFunction, ResponseSpectrum, ariasIntensityCreator, fourierTransform
from components.navbar import navbarComponent
from io import BytesIO
import base64

# set the title
processorTitle = dmc.Text("🌐Earthquake Data Processor", className='fs-3 mx-4 mb-3 mt-3')
...
def visualizeData(contents, filename, fig, theme):
    if contents is None:
        return fig
...
def applyTrim(trimRange, fig, contents, filename, theme, clickApply, clickReset):
...
def applyDetrend(method, order, fig, theme, n_clicks):
    if not n_clicks:
        raise PreventUpdate
...
def applyFilter(click, filterMethod, lowpasscorner, highpasscorner, filterOrder, fig, theme):
...
def createResponseSpectrum(signalFig, responseFig, dampingRatio, theme, click):
    if click is None:
        raise PreventUpdate
...
def createAriasFigure(signalFig, ariasFig, theme, click):
    if click is None:
        raise PreventUpdate
...
def createFourierFigure(signalFig, fourierFig, theme, click):
    if click is None:
        raise PreventUpdate
...
@callback(
    [
        Output('signalFig', 'figure', allow_duplicate=True), 
        Output('analysisDivMantine', 'theme'),
        Output('defaultResponseFig', 'figure', allow_duplicate=True),
        Output('defaultAriasFigure', 'figure', allow_duplicate=True),
        Output('defaultFourierFig', 'figure', allow_duplicate=True),
    ],
    State('signalFig', 'figure'),
    State('defaultResponseFig', 'figure'),
    State('defaultAriasFigure', 'figure'),
...
def updateTheme(defaultSignalFig, defaultResFig, ariasFig, fourierFig, themeData):
    sFig = go.Figure(defaultSignalFig)
    rFig = go.Figure(defaultResFig)
    aFig = go.Figure(ariasFig)
    fFig = go.Figure(fourierFig)
    if themeData:
        sFig.update_layout(
        ...
        for trace in aFig.data:
            trace.line.color = 'blue'
        
        for annotation in aFig['layout']['annotations']:
    ...
    return sFig, tabTheme, rFig, aFig, fFig
